...
The cariable ROLE should be set to Leader, TERM should be set to 1. The next step is to check that all shards are in state IsolatedLeader and shardReady flag is set to true. After that, executing export on this node via restconf will collect all the data without the need for network communication.

...
where all yang files are backed up (during export) and loaded (during karaf startup). Json files that contain actual data are stored and read from
...
Since frinx daexim needs to start before all other ODL features, the featuresBoot specified in
...
Since karaf loads bundles of previously started features before loading featuresBoot, we must delete

> ${karaf.home}/data/cache

directory before every start. This can be automated by changing a line in the

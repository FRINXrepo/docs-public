...
Once import is done, karaf will continue booting other features. A consequence of running import before installing all bundles is that not all yang models are available. Thus, both import and export use the new folder ${karaf.home}/schemas where all yang files are backed up (during export) and loaded (during karaf startup). Json files that contain actual data are stored and read from ${karaf.home}/daexim folder.

## Enabling automatic import during startup

To enable automatic import, place the backed up json files into ${karaf.home}/daexim , yang schemas to ${karaf.home}/schemas and execute on all nodes:
...
Since frinx daexim needs to start before all other ODL features, the featuresBoot specified in ${karaf.home}/etc/org.apache.karaf.features.cfg must be split into two properties:
...
Since karaf loads bundles of previously started features before loading featuresBoot, we must delete ${karaf.home}/data/cache directory before every start. This can be automated by changing a line in the ${karaf.home}/etc/system.properties file:

    karaf.clean.cache=true
    

Note that all the changes mentioned above are already in place in the official Frinx distribution. If you are building your own karaf however, check that your etc folder has no unintended deviations from frinx karaf.
